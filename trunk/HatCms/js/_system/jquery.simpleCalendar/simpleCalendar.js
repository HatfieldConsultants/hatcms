...
/* Function to advance 1 day */
Date.prototype.nextDate = function() {
	return new Date(this.valueOf() + 24 * 60 * 60 * 1000);
}

/* Function to get all the Date Objects between the object and a provided endDate */
Date.prototype.getDateArray = function( endDate ) {
	var arr = new Array();
	if ( Object.prototype.toString.call(endDate) === '[object Date]' && isNaN(endDate.getTime()) == false ) {
		for ( var x = this.valueOf(); x.valueOf() <= endDate.valueOf(); x = new Date(x).nextDate().valueOf() ) {
			arr.push( new Date(x) );
		}
	}
	else {
...
/* Change the background color when the date has an event */
function simpleCalenderHighlightCell() {
	$('.sc-event').each( function() {
		var eventStrDate = new Date(Date.parse( $(this).find('span.sc-event-start').html() ));
		var eventEndDate = new Date(Date.parse( $(this).find('span.sc-event-end').html() ));
		var dateArray = eventStrDate.getDateArray( eventEndDate );

		for (var x = 0; x < dateArray.length; x++) {
			var tempDate = dateArray[x];

			$('.sc-day-number').each(function( index ) {
...
/* add a footer row showing the legend */
function simpleCalenderAddFooter( txtToday, txtEvents ) {
	$('div.sc-view-month > table > tbody').append(
		'<tr class="sc-week6">'+
		'	<td class="sc-today">'+
		'		<div class="sc-day-number">&#160;</div>'+
		'	</td>'+
		'	<td class="sc-not-today" colspan="2">'+
...
var setDefaults = sc.setDefaults = function(d) {
	$.extend(true, defaults, d);
};



/* .simpleCalendar jQuery function
-----------------------------------------------------------------------------*/
...
$.fn.simpleCalendar = function(options) {
	...
	if (typeof options == 'string') {
		var args = Array.prototype.slice.call(arguments, 1),
			res;
		this.each(function() {
			var data = $.data(this, 'simpleCalendar');
			if (data) {
	...
	var tm = options.theme ? 'ui' : 'sc'; // for making theme classes
	...
	this.each(function() {
		...
		// element
		var _element = this,
			element = $(_element).addClass('sc'),
			elementOuterWidth,
			content = $("<div class='sc-content " + tm + "-widget-content' style='position:relative'/>").prependTo(_element),
			suggestedViewHeight,
			resizeUID = 0,
		...
		function changeView(v) {
			if (v != viewName) {
				...
				if (viewInstances[v]) {
					...
					view = viewInstances[v] = sc.views[v](
						newViewElement = absoluteViewElement =
							$("<div class='sc-view sc-view-" + v + "' style='position:absolute'/>")
								.appendTo(content),
						options,
						v // the view's name
					);
				...
				if (header) {
					...
					header.find('div.sc-button-' + viewName).removeClass(tm + '-state-active');
					header.find('div.sc-button-' + v).addClass(tm + '-state-active');
				}
		...
		function render(inc) {
			if (elementVisible()) {
				...
				if (header) {
					// update title text
					header.find('h2.sc-header-title').html(view.title);
					// enable/disable 'today' button
					var today = new Date();
					if (today >= view.start && today < view.end) {
						header.find('div.sc-button-today').addClass(tm + '-state-disabled');
					}else{
						header.find('div.sc-button-today').removeClass(tm + '-state-disabled');
					}
				}
		...
		var publicMethods = {
			...
			destroy: function() {
				...
				content.remove();
				$.removeData(_element, 'simpleCalendar');
			},
		...
		$.data(this, 'simpleCalendar', publicMethods); // TODO: look into memory leak implications
		...
		if (sections) {
			header = $("<table class='sc-header'/>")
				.append($("<tr/>")
					.append($("<td class='sc-header-left'/>").append(buildSection(sections.left)))
					.append($("<td class='sc-header-center'/>").append(buildSection(sections.center)))
					.append($("<td class='sc-header-right'/>").append(buildSection(sections.right))))
				.prependTo(element);
		}
		function buildSection(buttonStr) {
			if (buttonStr) {
				...
				$.each(buttonStr.split(' '), function(i) {
					if (i > 0) {
						tr.append("<td><span class='sc-header-space'/></td>");
					}
					var prevButton;
					$.each(this.split(','), function(j, buttonName) {
						if (buttonName == 'title') {
							tr.append("<td><h2 class='sc-header-title'>&nbsp;</h2></td>");
							if (prevButton) {
							...
							if (buttonClick) {
								...
								if (icon) {
									button = $("<div class='sc-button-" + buttonName + " ui-state-default'>" +
										"<a><span class='ui-icon ui-icon-" + icon + "'/></a></div>");
								}
								else if (text) {
									button = $("<div class='sc-button-" + buttonName + " " + tm + "-state-default'>" +
		...
		if (options.droppable) {
			...
			$(document)
				.bind('dragstart', function(ev, ui) {
					var _e = ev.target;
					var e = $(_e);
					if (!e.parents('.sc').length) { // not already inside a calendar
...
function normalizeEvent(event, options) {
	event._id = event._id || (event.id === undefined ? '_sc' + fakeID++ : event.id + '');
	if (event.date) {
		if (!event.start) {
...
function Grid(element, options, methods, viewName) {
	...
	disableTextSelection(element.addClass('sc-grid'));
	...
	function renderGrid(r, c, colFormat, showNumbers) {
		...
		if (!tbody) { // first time, build all cells from scratch
			...
			for (i=0; i<rowCnt; i++) {
				s += "<tr class='sc-week" + i + "'>";
				for (j=0; j<colCnt; j++) {
					s += "<td class='sc-" +
						dayIDs[d.getDay()] + ' ' + // needs to be first
						tm + '-state-default sc-day' + (i*colCnt+j) +
						(j==dit ? ' sc-leftmost' : '') +
						(rowCnt>1 && d.getMonth() != month ? ' sc-other-month' : '') +
						(+d == +today ?
						' sc-today '+tm+'-state-highlight' :
						' sc-not-today') + "'>" +
/* HatCms */				(showNumbers ? "<div class='sc-day-number' title='" + d.getFullYear() + "/" + (d.getMonth()+1) + "/" + d.getDate() + "'>" + d.getDate() + "</div>" : '') +
						"<div class='sc-day-content'><div style='position:relative'>&nbsp;</div></div></td>";
					addDays(d, 1);
					if (nwe) {
						skipWeekend(d);
					}
			...
			if (rowCnt < prevRowCnt) {
				...
				for (i=prevRowCnt; i<rowCnt; i++) {
					...
					for (j=0; j<colCnt; j++) {
						s += "<td class='sc-" +
							dayIDs[d.getDay()] + ' ' + // needs to be first
							tm + '-state-default sc-new sc-day' + (i*colCnt+j) +
							(j==dit ? ' sc-leftmost' : '') + "'>" +
/* HatCms */					(showNumbers ? "<div class='sc-day-number' title='" + d.getFullYear() + "/" + (d.getMonth()+1) + "/" + d.getDate() + "'></div>" : '') +
							"<div class='sc-day-content'><div style='position:relative'>&nbsp;</div></div>" +
							"</td>";
						addDays(d, 1);
						if (nwe) {
							skipWeekend(d);
			...
			dayBind(tbody.find('td.sc-new').removeClass('sc-new'));
			...
			tbody.find('td').each(function() {
				...
				if (rowCnt > 1) {
					if (d.getMonth() == month) {
						td.removeClass('sc-other-month');
					}else{
						td.addClass('sc-other-month');
					}
				}
				if (+d == +today) {
					td.removeClass('sc-not-today')
						.addClass('sc-today')
						.addClass(tm + '-state-highlight');
				}else{
					td.addClass('sc-not-today')
						.removeClass('sc-today')
...
/* HatCms */	td.find('div.sc-day-number').css('background-color','#e4ded7');
			...
			if (rowCnt == 1) { // more changes likely (week or day view)
				...
				thead.find('th').each(function() {
					...
					this.className = this.className.replace(/^sc-\w+(?= )/, 'sc-' + dayIDs[d.getDay()]);
					addDays(d, 1);
					if (nwe) {
						skipWeekend(d);
					}
	...
	function dayClick(ev) {
		if (!view.option('selectable')) { // SelectionManager will worry about dayClick
			var n = parseInt(this.className.match(/sc\-day(\d+)/)[1]),
				date = addDays(
...
function _renderDaySegs(segs, rowCnt, view, minLeft, maxLeft, getRow, dayContentLeft, dayContentRight, segmentContainer, bindSegHandlers, modifiedEventId) {
	...
	for (i=0; i<segCnt; i++) {
		seg = segs[i];
		event = seg.event;
		className = 'sc-event sc-event-hori ';
		if (rtl) {
			if (seg.isStart) {
				className += 'sc-corner-right ';
			}
			if (seg.isEnd) {
				className += 'sc-corner-left ';
			}
			...
			"<div class='" + className + event.className.join(' ') + "' style='position:absolute;z-index:8;left:"+left+"px'>" +
				"<a" + (event.url ? " href='" + htmlEscape(event.url) + "'" : '') + ">" +
					(!event.allDay && seg.isStart ?
						"<span class='sc-event-time'>" +
							htmlEscape(formatDates(event.start, event.end, view.option('timeFormat'), options)) +
						"</span>"
					:'') +
					"<span class='sc-event-title'>" + htmlEscape(event.title) + "</span>" +
					"<span class='sc-event-start'>" + event.start + "</span>" +
					"<span class='sc-event-end'>" + event.end + "</span>" +
				"</a>" +
	...
	for (i=0, rowI=0; rowI<rowCnt; rowI++) {
		...
		rowDivs[rowI] = getRow(rowI).find('td:first div.sc-day-content > div') // optimal selector?
...
function Agenda(element, options, methods, viewName) {
	...
	disableTextSelection(element.addClass('sc-agenda'));
	...
	function renderAgenda(c, colFormat) {
		...
		if (!head) { // first time rendering, build from scratch
			...
			// head
			s = "<div class='sc-agenda-head' style='position:relative;z-index:4'>" +
				"<table style='width:100%'>" +
				"<tr class='sc-first" + (options.allDaySlot ? '' : ' sc-last') + "'>" +
				"<th class='sc-leftmost " +
					tm + "-state-default'>&nbsp;</th>";
			for (i=0; i<colCnt; i++) {
				s += "<th class='sc-" +
					dayIDs[d.getDay()] + ' ' + // needs to be first
			...
			if (options.allDaySlot) {
				s += "<tr class='sc-all-day'>" +
						"<th class='sc-axis sc-leftmost " + tm + "-state-default'>" + options.allDayText + "</th>" +
						"<td colspan='" + colCnt + "' class='" + tm + "-state-default'>" +
							"<div class='sc-day-content'><div style='position:relative'>&nbsp;</div></div></td>" +
						"<th class='" + tm + "-state-default'>&nbsp;</th>" +
					"</tr><tr class='sc-divider sc-last'><th colspan='" + (colCnt+2) + "' class='" +
						tm + "-state-default sc-leftmost'><div/></th></tr>";
			}
			s+= "</table></div>";
			...
			for (i=0; d < maxd; i++) {
				...
				s += "<tr class='" +
					(!i ? 'sc-first' : (!minutes ? '' : 'sc-minor')) +
					"'><th class='sc-axis sc-leftmost " + tm + "-state-default'>" +
					((!slotNormal || !minutes) ? formatDate(d, options.axisFormat) : '&nbsp;') + 
					"</th><td class='sc-slot" + i + ' ' +
						tm + "-state-default'><div style='position:relative'>&nbsp;</div></td></tr>";
				addMinutes(d, options.slotMinutes);
				slotCnt++;
			}
			s += "</table>";
			body = $("<div class='sc-agenda-body' style='position:relative;z-index:2;overflow:auto'/>")
			...
			// background stripes
			d = cloneDate(d0);
			s = "<div class='sc-agenda-bg' style='position:absolute;z-index:1'>" +
				"<table style='width:100%;height:100%'><tr class='sc-first'>";
			for (i=0; i<colCnt; i++) {
				s += "<td class='sc-" +
					dayIDs[d.getDay()] + ' ' + // needs to be first
					tm + '-state-default ' +
					(!i ? 'sc-leftmost ' : '') +
					(+d == +today ? tm + '-state-highlight sc-today' : 'sc-not-today') +
					"'><div class='sc-day-content'><div>&nbsp;</div></div></td>";
				addDays(d, dis);
				if (nwe) {
					skipWeekend(d, dis);
				}
			...
			bg.find('td').each(function() {
				...
				if (+d == +today) {
					$(this)
						.removeClass('sc-not-today')
						.addClass('sc-today')
						.addClass(tm + '-state-highlight');
				}else{
					$(this)
						.addClass('sc-not-today')
						.removeClass('sc-today')
						.removeClass(tm + '-state-highlight');
				}
				addDays(d, dis);
	...
	function slotClick(ev) {
		if (!view.option('selectable')) { // SelectionManager will worry about dayClick
			var col = Math.min(colCnt-1, Math.floor((ev.pageX - bg.offset().left) / colWidth)),
				...
				rowMatch = this.className.match(/sc-slot(\d+)/);
			if (rowMatch) {
	...
	function renderDaySegs(segs, modifiedEventId) {
		if (options.allDaySlot) {
			_renderDaySegs(
				...
				viewWidth,
				function() {
					return head.find('tr.sc-all-day');
				},
				function(dayOfWeek) {
	...
	function renderSlotSegs(segs, modifiedEventId) {
		...
		for (i=0; i<segCnt; i++) {
			seg = segs[i];
			event = seg.event;
			className = 'sc-event sc-event-vert ';
			if (seg.isStart) {
				className += 'sc-corner-top ';
			}
			if (seg.isEnd) {
				className += 'sc-corner-bottom ';
			}
		...
		for (i=0; i<segCnt; i++) {
			...
			if (eventElement = seg.element) {
				...
				val = hsideCache[key];
				seg.hsides = val === undefined ? (hsideCache[key] = hsides(eventElement[0], true)) : val;
				titleSpan = eventElement.find('span.sc-event-title');
				if (titleSpan.length) {
		...
		for (i=0; i<segCnt; i++) {
			...
			if (eventElement = seg.element) {
				...
				if (seg.titleTop !== undefined && height - seg.titleTop < 10) {
					...
					eventElement.find('span.sc-event-time')
						.text(formatDate(event.start, view.option('timeFormat')) + ' - ' + event.title);
					eventElement.find('span.sc-event-title')
						.remove();
				}
				view.trigger('eventAfterRender', event, event, eventElement);
			}
	...
	function slotSegHtml(event, seg, className) {
		return "<div class='" + className + event.className.join(' ') + "' style='position:absolute;z-index:8;top:" + seg.top + "px;left:" + seg.left + "px'>" +
			"<a" + (event.url ? " href='" + htmlEscape(event.url) + "'" : '') + ">" +
				"<span class='sc-event-bg'></span>" +
				"<span class='sc-event-time'>" + htmlEscape(formatDates(event.start, event.end, view.option('timeFormat'))) + "</span>" +
				"<span class='sc-event-title'>" + htmlEscape(event.title) + "</span>" +
				"<span class='sc-event-start'>" + event.start + "</span>" +
				"<span class='sc-event-end'>" + event.end + "</span>" +
			"</a>" +
	...
	function slotSegBind(event, eventElement, seg) {
		view.eventElementHandlers(event, eventElement);
		if (event.editable || event.editable === undefined && options.editable) {
			var timeElement = eventElement.find('span.sc-event-time');
		...
		if (!options.disableDragging && eventElement.draggable) {
			...
			eventElement.draggable({
				...
				start: function(ev, ui) {
					...
					view.hideEvents(event, eventElement);
					if ($.browser.msie) {
						eventElement.find('span.sc-event-bg').hide(); // nested opacities mess up in IE, just hide
					}
				...
				if (bottom > top) { // protect against selections that are entirely before or after visible range
					...
					if ($.isFunction(helperOption)) {
						...
						selectionHelper = $(slotSegHtml(
							...
							rect,
							'sc-event sc-event-vert sc-corner-top sc-corner-bottom '
						));
						if ($.browser.msie) {
							selectionHelper.find('span.sc-event-bg').hide(); // nested opacities mess up in IE, just hide
						}
...
var viewMethods = {
	...
	renderOverlay: function(rect, parent) {
		...
		if (!e) {
			e = $("<div class='sc-cell-overlay' style='position:absolute;z-index:3'/>");
		}
...
var parseDate = sc.parseDate = function(s) {
...
var parseISO8601 = sc.parseISO8601 = function(s, ignoreTimezone) {
...
var parseTime = sc.parseTime = function(s) { // returns minutes since start of day
...
var formatDate = sc.formatDate = function(date, format, options) {
	return formatDates(date, null, format, options);
};

var formatDates = sc.formatDates = function(date1, date2, format, options) {
