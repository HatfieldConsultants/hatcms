...
namespace Hatfield.Web.Portal.MetaData
{
    ///
    ///			http://www.goheer.com
    ...
    public class EXIFextractor : IEnumerable
    {
        ...
        private static PropertyItem CreatePropertyItem(short type, int tag, int len, byte[] value)
        {
            ...
            Stream emptyBitmapStream = null;
            foreach (string resName in Assembly.GetExecutingAssembly().GetManifestResourceNames())
            {
                if (resName.ToLower().EndsWith("decoy.jpg"))
                {
                    emptyBitmapStream = assembly.GetManifestResourceStream(resName);
                }
            }

            if (emptyBitmapStream == null)
                throw new Exception("Error: decoy.jpg must be embedded as a resource into your project!");
    ...
    class EXIFextractorEnumerator : IEnumerator
    {
        ...
        public object Current
        {
            get
            {
                return (new PairOfObjects(this.index.Key, this.index.Value));
            }
        }
    ...
    public class PairOfObjects
    {
        ...
        public PairOfObjects(object first, object second)
        {
            this.First = first;
            this.Second = second;
        }
