/*
This plugin over-rides the existing CKEditor styles plugin. 

The out-of-the-box styles plugin has an un-desirable way of handling DIV styles.
The standard behaviour is to take all selected block elements and convert them into DIVs,
rather than surrounding them with a DIV.

As such, <p>Para 1</p><p>Para 2</p><p>Para 3</p> becomes
<div>Para 1</div><div>Para 2</div><div>Para 3</div>

This modified plugin modifies this behaviour to surround existing blocks
with a new DIV, rather than converting existing block elements into DIVs.

Source for DIV handling comes mostly from the DIV ckeditor plugin.

Note: when using this plugin, the 'styles' plugin must be added to the config.removePlugins list
eg: config.removePlugins = 'styles';

-- Jason Suwala April 30, 2010.

Updated June 3, 2011 to use code from CKEditor v3.6
-- Jason Suwala June 3, 2011

Copyright (c) 2003-2010, CKSource - Frederico Knabben. All rights reserved.
For licensing, see LICENSE.html or http://ckeditor.com/license
*/

CKEDITOR.plugins.add( 'hatCms_styles',
{
	requires : [ 'selection', 'domiterator' ],
...
CKEDITOR.editor.prototype.attachStyleStateChange = function( style, callback )
{
	...
	if ( !styleStateChangeCallbacks )
	{
		...
		this.on( 'selectionChange', function( ev )
			{
				// alert('hatCms_styles selectionChange');
...
(function()
{
	...
	// This function was modified to work with HatCms
	function applyBlockStyle( range )
	{
		...
		if (this.element == 'div')
		{
			// alert('New hatCms div handling');
			var div = getDiv( CKEDITOR.currentInstance );			
			// alert(div);
			if (div && this.checkElementRemovable(div))
			{
				// alert('do nothing');
			...
			else
			{
				...
				var containers = createDiv(CKEDITOR.currentInstance);
				for(var i=0; i < containers.length; i++)
				{		
					replaceBlock( containers[i], newBlock );
				}
			}
		}
		else
	...
	// JS: new function; copied from div.js
	function getDiv( editor )
	{
		...
		var divLimitDefinition = ( function(){
			...
			if( true || editor.config.div_wrapTable )
			{
